...
const WoodSlicer = () => {
    ...
    try {
      ...
      operations.forEach((op: SavedOperation) => {
        ...
        if (op.lot_number === lotNumber) {
          ...
          // Only mark as "used" if the operation has planks
          if (op.sleeper_number && op.plank_sizes && op.plank_sizes.length > 0) {
  ...
  const handleSleeperSelect = async (sleeper: any) => {
    ...
    // Reset state first to prevent carrying over data from previous sleeper
    setOperationId(null);
  ...
  const addPlankSize = async (sleeperSequence: number) => {
    ...
    if (!operationId && selectedWoodType && lotNumber && selectedSleeperNumber) {
      ...
      console.log('usedSleeperNumbers:', Array.from(usedSleeperNumbers));
      // Small delay to let state update
      setTimeout(() => {
        handleSave(false);
      }, 100);
    } else if (operationId) {
      console.log('Operation already exists, operationId:', operationId);
    }
  };
  ...
  const handleSave = async (isAutoSave = false) => {
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        // Don't save if there are no planks
        if (plankSizes.length === 0) {
          if (isAutoSave) {
            // Skip auto-save if no planks added yet
            return null;
          } else {
            // Show error for manual save
            throw new Error('Cannot save operation without any planks. Please add at least one plank.');
          }
        }
        ...
        console.log('handleSave - operationId:', operationId);
        console.log('handleSave - selectedSleeperNumber:', selectedSleeperNumber);
        console.log('handleSave - lotNumber:', lotNumber);

        if (operationId) {
          // Updating existing operation - no duplicate check needed
          console.log('Updating existing operation:', operationId);
          ...
          console.log('Creating new operation, checking for duplicates...');
          if (selectedSleeperNumber && lotNumber) {
            ...
            console.log('Existing operations:', existingOps.filter((op: any) => op.lot_number === lotNumber).map((op: any) => ({id: op.id, sleeper: op.sleeper_number})));
            const duplicate = existingOps.find(
              (op: any) => op.lot_number === lotNumber && op.sleeper_number === selectedSleeperNumber
            );

            if (duplicate) {
              console.log('Found duplicate operation:', duplicate.id);
  ...
  const CompletedOperationsDialog = () => (
    ...
    >
      ...
      <DialogContent sx={{ p: 0 }}>
        {savedOperations.filter(op => op.lot_number === lotNumber && op.plank_sizes && op.plank_sizes.length > 0).length === 0 ? (
          ...
          <List sx={{ py: 0 }}>
            {savedOperations
              .filter(op => op.lot_number === lotNumber && op.plank_sizes && op.plank_sizes.length > 0)
            ...
            >
              ...
              <Stack spacing={0.5} sx={{ mt: 0.5 }}>
                <Typography variant="body2" sx={{ fontSize: '0.75rem', color: '#334155' }}>
                  Start: {selectedOperation.start_time ? format(new Date(selectedOperation.start_time), 'PPpp') : 'Not started'}
                </Typography>
                <Typography variant="body2" sx={{ fontSize: '0.75rem', color: '#334155' }}>
                  End: {selectedOperation.updated_at ? format(new Date(selectedOperation.updated_at), 'PPpp') : 'N/A'}
                </Typography>
                <Typography
                  variant="subtitle2"
                  sx={{
          ...
          >
            ...
            <Stack spacing={2.5}>
              {sleeperSizes.map((sleeper, sleeperIndex) => (
                ...
                >
                  <Box sx={{ mb: 2.5 }}>
                    {/* Responsive layout with square label and dimensions */}
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: { xs: 'wrap', sm: 'nowrap' } }}>
                      ...
                      >
                        <Typography sx={{ fontWeight: 700, fontSize: { xs: '0.65rem', sm: '0.75rem' }, opacity: 0.9 }}>
                          Sleeper
                        </Typography>
                        <Typography sx={{ fontWeight: 800, fontSize: { xs: '1.25rem', sm: '1.5rem' }, lineHeight: 1 }}>
                          #{selectedSleeperNumber || sleeper.sequence}
                        </Typography>
                        <Typography sx={{ fontWeight: 600, fontSize: { xs: '0.6rem', sm: '0.65rem' }, opacity: 0.8 }}>
                          SL{String(selectedSleeperNumber || sleeper.sequence).padStart(3, '0')}
                        </Typography>
                      </Box>

                      {/* Dimensions as styled text (not inputs) */}
                        ...
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                          <Box>
                            ...
                            <Typography sx={{ fontSize: '1rem', fontWeight: 700, color: '#1e293b' }}>
                              {cmToInches(sleeper.height || 0).toFixed(0)}" <Typography component="span" sx={{ fontSize: '0.75rem', color: '#64748b' }}>({sleeper.height?.toFixed(1)}cm)</Typography>
                            </Typography>
                          </Box>
                          <Box>
                            <Typography sx={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, mb: 0.5 }}>
                            ...
                            <Typography sx={{ fontSize: '1rem', fontWeight: 700, color: '#1e293b' }}>
                              {cmToInches(sleeper.width || 0).toFixed(0)}" <Typography component="span" sx={{ fontSize: '0.75rem', color: '#64748b' }}>({sleeper.width?.toFixed(1)}cm)</Typography>
                            </Typography>
                          </Box>
                          <Box>
                            <Typography sx={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, mb: 0.5 }}>
                            ...
                            <Typography sx={{ fontSize: '1rem', fontWeight: 700, color: '#1e293b' }}>
                              {metersToFeet(sleeper.length || 0).toFixed(0)}' <Typography component="span" sx={{ fontSize: '0.75rem', color: '#64748b' }}>({sleeper.length?.toFixed(2)}m)</Typography>
                            </Typography>
                          </Box>
                        </Box>
                      </Box>
                    ...
                    <Stack spacing={2}>
                      {plankSizes
                        ...
                        .map((plank) => (
                          ...
                          >
                            {/* Responsive layout with square plank label on left */}
                            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: { xs: 'wrap', sm: 'nowrap' } }}>
                              ...
                              >
                                ...
                                <Typography sx={{ fontWeight: 800, fontSize: { xs: '1.1rem', sm: '1.25rem' }, lineHeight: 1 }}>
                                  #{plankSizes.filter(p => p.parentSequence === sleeper.sequence).indexOf(plank) + 1}
                                </Typography>
                                <Typography sx={{ fontWeight: 600, fontSize: { xs: '0.55rem', sm: '0.65rem' }, opacity: 0.8 }}>
                              ...
                              {/* Dimension Inputs - 2x2 grid layout */}
                              <Box sx={{ flex: 1, minWidth: 0 }}>
                                {/* Row 1: Height and Width */}
                                <Box sx={{
                                  display: 'grid',
